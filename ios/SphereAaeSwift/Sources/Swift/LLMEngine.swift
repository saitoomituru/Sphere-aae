...
public class SphereAaeEngine {
    ...
    // we decouple it from SphereAaeEngine
    ...
    actor EngineState {
        ...
        func streamCallback(result: String?) {
            ...
            // Todo(sphere-aae-team): check the last error in engine and report if there's any
        }
    }
    ...
    public class Completions {
        ...
        public func create(
            ...
            if !stream {
                state.logger.error("Only stream=true is supported in SphereAaeSwift")
            }
            let request = ChatCompletionRequest(
                messages: messages,
                model: model,
