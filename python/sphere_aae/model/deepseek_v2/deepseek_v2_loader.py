...
from sphere_aae.loader import ExternMapping, QuantizeMapping
from sphere_aae.quantization import BlockScaleQuantize, Quantization
...
def huggingface(  # pylint: disable=too-many-locals,too-many-statements
    ...
    def add_weight_and_scale_mapping(
        weight_sphere_aae_name: str,
        weight_hf_names: List[str],
        weight_transform_func: Callable,
    ):
        sphere_aae_param = named_parameters[weight_sphere_aae_name]
        mapping.add_mapping(
            weight_sphere_aae_name,
            weight_hf_names,
            functools.partial(weight_transform_func, dtype=sphere_aae_param.dtype),
        )

        if isinstance(quantization, BlockScaleQuantize):
            scale_sphere_aae_name = f"{weight_sphere_aae_name}_scale_inv"
            if scale_sphere_aae_name in named_parameters:
                scale_hf_names = [f"{name}_scale_inv" for name in weight_hf_names]
                scale_param = named_parameters[scale_sphere_aae_name]
                mapping.add_mapping(
                    scale_sphere_aae_name,
                    scale_hf_names,
    ...
    for i in range(model_config.num_hidden_layers):
        if i >= model_config.first_k_dense_replace and i % model_config.moe_layer_freq == 0:
            ...
            if model_config.topk_method == "noaux_tc":
                sphere_aae_name = f"{mlp}.e_score_correction_bias"
        ...
        if isinstance(quantization, BlockScaleQuantize):
            scale_sphere_aae_name = f"{attn}.w_uk_scale_inv"
            sphere_aae_param = named_parameters[scale_sphere_aae_name]
            mapping.add_mapping(
                scale_sphere_aae_name,
        ...
        if isinstance(quantization, BlockScaleQuantize):
            scale_sphere_aae_name = f"{attn}.w_uv_scale_inv"
            sphere_aae_param = named_parameters[scale_sphere_aae_name]
            mapping.add_mapping(
                scale_sphere_aae_name,
                [f"{attn}.kv_b_proj.weight_scale_inv"],
