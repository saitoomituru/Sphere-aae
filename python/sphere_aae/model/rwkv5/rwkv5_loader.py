...
def huggingface(model_config: RWKV5Config, quantization: Quantization) -> ExternMapping:
    ...
    for i in range(model_config.num_hidden_layers):
        # convert time_decay
        sphere_aae_name = f"model.blocks.{i}.attention.time_decay"
        hf_name = f"rwkv.blocks.{i}.attention.time_decay"
        sphere_aae_param = named_parameters[sphere_aae_name]
        if sphere_aae_param.dtype != "float32":
            raise ValueError(f"RWKV5 time_decay should be float32, got {sphere_aae_param.dtype}")
        mapping.add_mapping(
            sphere_aae_name,
            [hf_name],
            functools.partial(
        ...
        if model_config.rescale_every > 0:
            for name in ["feed_forward.value.weight", "attention.output.weight"]:
                sphere_aae_name = f"model.blocks.{i}.{name}"
    ...
    for sphere_aae_name, sphere_aae_param in named_parameters.items():
        if sphere_aae_name not in mapping.param_map:
            hf_name = sphere_aae_name.replace("model", "rwkv")
            mapping.add_mapping(
                sphere_aae_name,
                [hf_name],
                functools.partial(
