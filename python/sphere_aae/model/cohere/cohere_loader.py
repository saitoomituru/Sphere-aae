...
def huggingface(model_config: CohereConfig, quantization: Quantization) -> ExternMapping:
    ...
    def _add(sphere_aae_name, hf_name):
        mapping.add_mapping(
            sphere_aae_name,
            [hf_name],
            functools.partial(
...
def awq(model_config: CohereConfig, quantization: Quantization) -> ExternMapping:
    ...
    for i in range(model_config.num_hidden_layers):
        ...
        for quantize_suffix in ["qweight", "qzeros", "scales"]:
            sphere_aae_name = f"{attn}.qkv_proj.{quantize_suffix}"
            assert sphere_aae_name in named_parameters
            sphere_aae_param = named_parameters[sphere_aae_name]
            mapping.add_mapping(
                sphere_aae_name,
                [
    ...
    for sphere_aae_name, sphere_aae_param in named_parameters.items():
        if sphere_aae_name not in mapping.param_map:
            mapping.add_mapping(
                sphere_aae_name,
                [sphere_aae_name],
                functools.partial(lambda x, dtype: x.astype(dtype), dtype=sphere_aae_param.dtype),
            )
    return mapping
