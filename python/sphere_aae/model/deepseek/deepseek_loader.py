...
def huggingface(model_config: DeepseekConfig, quantization: Quantization) -> ExternMapping:
    ...
    for i in range(model_config.num_hidden_layers):
        ...
        for weight_type in ["weight"]:
            sphere_aae_name = f"{attn}.wqkv_pack.{weight_type}"
    ...
    for i in range(model_config.num_hidden_layers):
        if i >= model_config.first_k_dense_replace and i % model_config.moe_layer_freq == 0:
            ...
            sphere_aae_name = f"{shared_expert}.gate_up_proj.weight"
            sphere_aae_param = named_parameters[sphere_aae_name]
            mapping.add_mapping(
                sphere_aae_name,
                [
            ...
            sphere_aae_name = f"{mlp}.moe_gate_up_proj.weight"
            ...
            # map mlp moe gate and up weight
            sphere_aae_name = f"{mlp}.moe_down_proj.weight"
            sphere_aae_param = named_parameters[sphere_aae_name]
            mapping.add_mapping(
                sphere_aae_name,
                [
