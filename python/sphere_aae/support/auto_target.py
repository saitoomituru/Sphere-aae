...
from .constants import SPHERE_AAE_MULTI_ARCH
from .style import bold, green, red

if TYPE_CHECKING:
    from sphere_aae.compiler.compile import CompileArgs
...
def _build_webgpu():
    def build(mod: IRModule, args: "CompileArgs", pipeline=None):
        ...
        # Try to locate `sphere_aae_wasm_runtime.bc`
        bc_path = None
        bc_candidates = ["web/dist/wasm/sphere_aae_wasm_runtime.bc"]
        if os.environ.get("SPHERE_AAE_SOURCE_DIR", None):
            sphere_aae_source_home_dir = os.environ["SPHERE_AAE_SOURCE_DIR"]
            bc_candidates.append(
                os.path.join(sphere_aae_source_home_dir, "web", "dist", "wasm", "sphere_aae_wasm_runtime.bc")
            )
        error_info = (
            "Cannot find library: sphere_aae_wasm_runtime.bc\n"
            + "Make sure you have run `./web/prep_emcc_deps.sh` and "
            + "`export SPHERE_AAE_SOURCE_DIR=/path/to/sphere-aae` so that we can locate the file. "
...
def detect_cuda_arch_list(target: Target) -> List[int]:
    ...
    if SPHERE_AAE_MULTI_ARCH is not None:
        multi_arch = [convert_to_num(x) for x in SPHERE_AAE_MULTI_ARCH.split(",")]
    else:
...
def _register_cuda_hook(target: Target):
    if SPHERE_AAE_MULTI_ARCH is None:
        ...
        logger.info(
            ...
            "Example: SPHERE_AAE_MULTI_ARCH=70,72,75,80,86,87,89,90a",
            bold("SPHERE_AAE_MULTI_ARCH"),
        )
        multi_arch = None
    else:
        logger.info("%s %s: %s", FOUND, bold("SPHERE_AAE_MULTI_ARCH"), SPHERE_AAE_MULTI_ARCH)
        multi_arch = [x.strip() for x in SPHERE_AAE_MULTI_ARCH.split(",")]
