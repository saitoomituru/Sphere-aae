...
from .constants import (
    SPHERE_AAE_DOWNLOAD_CACHE_POLICY,
    SPHERE_AAE_HOME,
    SPHERE_AAE_READONLY_WEIGHT_CACHE,
    SPHERE_AAE_TEMP_DIR,
)
...
def log_download_cache_policy():
    ...
    logger.info(
        ...
        bold("SPHERE_AAE_DOWNLOAD_CACHE_POLICY"),
        SPHERE_AAE_DOWNLOAD_CACHE_POLICY,
    )
...
def download_and_cache_sphere_aae_weights(  # pylint: disable=too-many-locals
    model_url: str,
    num_processes: int = 4,
    ...
    if SPHERE_AAE_DOWNLOAD_CACHE_POLICY == "OFF":
        raise RuntimeError(f"Cannot download {model_url} as SPHERE_AAE_DOWNLOAD_CACHE_POLICY=OFF")

    prefixes, sphere_aae_prefix = ["HF://", "https://huggingface.co/"], ""
    sphere_aae_prefix = next(p for p in prefixes if model_url.startswith(p))
    assert sphere_aae_prefix

    git_url_template = "https://huggingface.co/{user}/{repo}"
    bin_url_template = "https://huggingface.co/{user}/{repo}/resolve/main/{record_name}"

    if model_url.count("/") != 1 + sphere_aae_prefix.count("/") or not model_url.startswith(sphere_aae_prefix):
        raise ValueError(f"Invalid model URL: {model_url}")
    user, repo = model_url[len(sphere_aae_prefix) :].split("/")
    domain = "hf"

    readonly_cache_dirs = []
    for base in SPHERE_AAE_READONLY_WEIGHT_CACHE:
        cache_dir = base / domain / user / repo
        readonly_cache_dirs.append(str(cache_dir))
        if (cache_dir / "sphere-aae-chat-config.json").is_file():
            logger.info("Use cached weight: %s", bold(str(cache_dir)))
            return cache_dir

    if force_redo is None:
        force_redo = SPHERE_AAE_DOWNLOAD_CACHE_POLICY == "REDO"

    git_dir = SPHERE_AAE_HOME / "model_weights" / domain / user / repo
    ...
    if SPHERE_AAE_DOWNLOAD_CACHE_POLICY == "READONLY":
        raise RuntimeError(
            f"Cannot find cache for {model_url}, "
            "cannot proceed to download as SPHERE_AAE_DOWNLOAD_CACHE_POLICY=READONLY, "
            "please check settings SPHERE_AAE_READONLY_WEIGHT_CACHE, "
            f"local path candidates: {readonly_cache_dirs}"
        )

    with tempfile.TemporaryDirectory(dir=SPHERE_AAE_TEMP_DIR) as tmp_dir_prefix:
...
def get_or_download_model(model: str) -> Path:
    ...
    We define "valid" as having an ``sphere-aae-chat-config.json`` right under the folder.
    ...
    model_path : Path
        ...
        ``(model_path / "sphere-aae-chat-config.json").is_file`` being True
    ...
    if model.startswith("HF://"):
        ...
        model_path = download_and_cache_sphere_aae_weights(model)
    else:
    ...
    if sphere_aae_config_path.is_file():
        return model_path
    raise FileNotFoundError(f"Cannot find {str(sphere_aae_config_path)} in the model directory provided")
