...
class DebugChat:  # pylint: disable=too-many-instance-attributes, too-few-public-methods
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        ----------
        model: str
            The model folder after compiling with Sphere-aae build process. The parameter
        ...
        self.config_file_path = self.model_path / "sphere-aae-chat-config.json"
        with open(self.config_file_path, mode="rt", encoding="utf-8") as file:
            self.chat_config = SphereAaeChatConfig.model_validate_json(file.read())
...
def main():
    ...
    parser = ArgumentParser("Astro Agent Edge (AAE) Chat Debug Tool")
    parser.add_argument(
        "prompt",
        type=str,
    ...
    parser.add_argument(
        "--model",
        type=str,
        help="An MLC model directory that contains `sphere-aae-chat-config.json`",
        required=True,
    )
    parser.add_argument(
        "--model-lib",
