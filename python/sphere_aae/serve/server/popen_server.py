"""The Astro Agent Edge (AAE) server launched in a subprocess."""
...
from sphere_aae.serve.engine_base import _check_engine_config


class PopenServer:  # pylint: disable=too-many-instance-attributes
    """The wrapper of Astro Agent Edge (AAE) server, which runs the server in
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        *,
        ...
        """Please check out `python/sphere_aae/cli/serve.py` for the server arguments."""
    ...
    def start(  # pylint: disable=too-many-branches,too-many-statements
        ...
        cmd = [sys.executable]
        cmd += ["-m", "sphere_aae", "serve", self.model]
        ...
        if process_return_code is not None:
            raise RuntimeError(
                ...
                f'Please check if "{self.model}" is a valid model compiled by Astro Agent Edge (AAE).'
            )
        if attempts == timeout:
            self.terminate()
