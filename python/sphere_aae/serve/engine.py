"""The Astro Agent Edge (AAE) Serving Engine."""
...
from sphere_aae.protocol import debug_protocol, openai_api_protocol
...
from sphere_aae.tokenizers import TextStreamer
...
class AsyncChat:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, engine: weakref.ReferenceType) -> None:
        assert isinstance(engine(), AsyncSphereAaeEngine)
...
class Chat:  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, engine: weakref.ReferenceType) -> None:
        assert isinstance(engine(), SphereAaeEngine)
...
class AsyncChatCompletion:  # pylint: disable=too-few-public-methods
    ...
    if sys.version_info >= (3, 9):
        engine: weakref.ReferenceType["AsyncSphereAaeEngine"]
    else:
    ...
    async def create(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        *,
        ...
        stream_response : ChatCompletionStreamResponse
            ...
            See sphere_aae/protocol/openai_api_protocol.py or
...
class ChatCompletion:  # pylint: disable=too-few-public-methods
    ...
    if sys.version_info >= (3, 9):
        engine: weakref.ReferenceType["SphereAaeEngine"]
    else:
        engine: weakref.ReferenceType
...
class AsyncSphereAaeEngine(engine_base.SphereAaeEngineBase):
    """The AsyncSphereAaeEngine in Astro Agent Edge (AAE) that provides the asynchronous
    interfaces with regard to OpenAI API.

    Parameters
    ----------
    model : str
        A path to ``sphere-aae-chat-config.json``, or an MLC model directory that contains
    ...
    mode : Literal["local", "interactive", "server"]
        The engine mode in Astro Agent Edge (AAE).
    ...
    async def _generate(
        ...
        """Internal asynchronous text generation interface of AsyncSphereAaeEngine.
...
class SphereAaeEngine(engine_base.SphereAaeEngineBase):
    """The SphereAaeEngine in Astro Agent Edge (AAE) that provides the synchronous
    interfaces with regard to OpenAI API.
    ...
    def _generate(  # pylint: disable=too-many-locals
        ...
        """Internal synchronous text generation interface of SphereAaeEngine.
        The method is a coroutine that streams a list of CallbackStreamOutput
        at a time via yield. The returned list length is the number of
        parallel generations specified by `generation_config.n`
    ...
    def _request_stream_callback_impl(
        ...
        """The underlying implementation of request stream callback of SphereAaeEngine."""
