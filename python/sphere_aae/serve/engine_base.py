"""The Astro Agent Edge (AAE) Serving engine base class."""
...
from sphere_aae.protocol.conversation_protocol import Conversation
from sphere_aae.protocol.generation_config import GenerationConfig
from sphere_aae.protocol.sphere_aae_chat_config import SphereAaeChatConfig
from sphere_aae.serve import data, engine_utils
from sphere_aae.serve.config import EngineConfig
from sphere_aae.serve.event_trace_recorder import EventTraceRecorder
from sphere_aae.support import download_cache, logging
from sphere_aae.support.auto_device import detect_device
from sphere_aae.support.style import green
from sphere_aae.tokenizers import TextStreamer, Tokenizer
...
class ModelInfo:
    ...
    model : str
        ...
        (e.g., "dist/prebuilt/sphere-aae-chat-Llama-2-7b-chat-hf-q4f16_1")
...
def _process_model_args(
    ...
    def _convert_model_info(model: ModelInfo) -> Tuple[str, str]:
        ...
        sphere_aae_config_path = model_path / "sphere-aae-chat-config.json"
        config_file_paths.append(str(sphere_aae_config_path))

        with open(sphere_aae_config_path, mode="rt", encoding="utf-8") as file:
            sphere_aae_chat_config = SphereAaeChatConfig.model_validate_json(file.read())

        if conversation is None:
            conversation = sphere_aae_chat_config.conv_template
        ...
        if model.model_lib is not None:
            ...
            from sphere_aae.interface import jit  # pylint: disable=import-outside-toplevel
...
class CallbackStreamOutput:
    """The output of SphereAaeEngine._generate and AsyncSphereAaeEngine._generate
...
class AsyncRequestStream:
    """The asynchronous stream for requests in AsyncSphereAaeEngine.
...
class EngineState:
    ...
    This class is used for both AsyncSphereAaeEngine and SphereAaeEngine.
    AsyncSphereAaeEngine uses the fields and methods starting with "async",
    and SphereAaeEngine uses the ones starting with "sync".

    - For AsyncSphereAaeEngine, the state contains an asynchronous event loop,
    the streamers and the number of unfinished generations for each request
    being processed.
    - For SphereAaeEngine, the state contains a callback output blocking queue,
    ...
    trace_recorder = None
    # States used for AsyncSphereAaeEngine
    ...
    # States used for SphereAaeEngine
    ...
    def _async_request_stream_callback(self, delta_outputs: List[data.RequestStreamOutput]) -> None:
        """The request stream callback function for AsyncSphereAaeEngine to stream back
    ...
    def _async_request_stream_callback_impl(
        ...
        """The underlying implementation of request stream callback for AsyncSphereAaeEngine."""
    ...
    def _sync_request_stream_callback(self, delta_outputs: List[data.RequestStreamOutput]) -> None:
        """The request stream callback function for SphereAaeEngine to stream back
        the request generation results.
        """
...
class SphereAaeEngineBase:  # pylint: disable=too-many-instance-attributes,too-few-public-methods
    """The base engine class, which implements common functions that
    are shared by SphereAaeEngine and AsyncSphereAaeEngine.
    ...
    SphereAaeEngine and AsyncSphereAaeEngine inherits this SphereAaeEngineBase class, and implements
    ...
    Checkout subclasses AsyncSphereAaeEngine/SphereAaeEngine for the docstring of constructor parameters.
    """
    ...
    def __init__(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        module = tvm.get_global_func("sphere_aae.serve.create_threaded_engine", allow_missing=False)()
        self._ffi = {
            key: module[key]
            for key in [
                "add_request",
