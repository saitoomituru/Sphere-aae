...
""".strip(),
    "model": """
A path to ``sphere-aae-chat-config.json``, or an MLC model directory that contains `sphere-aae-chat-config.json`.
...
""".strip(),
    "model_type": """
Model architecture such as "llama". If not set, it is inferred from `sphere-aae-chat-config.json`.
""".strip(),
    "device_compile": """
...
""".strip(),
    "opt": """
Optimization flags. Astro Agent Edge (AAE) maintains a predefined set of optimization flags,
...
""".strip(),
    "output_gen_sphere_aae_chat_config": """
The output directory for generated configurations, including `sphere-aae-chat-config.json` and tokenizer
configuration.
""".strip(),
    "sliding_window_size": """
...
""".strip(),
    "overrides": """
Model configuration override. Configurations to override `sphere-aae-chat-config.json`. Supports
...
""".strip(),
    "mode_serve": """
The engine mode in Astro Agent Edge (AAE). We provide three preset modes: "local", "interactive" and "server".
...
Please check out the documentation of EngineConfig in sphere_aae/serve/config.py for detailed docstring
of each field.
Example: --overrides "max_num_sequence=32;max_total_seq_length=4096;tensor_parallel_shards=2"
""".strip(),
    "config_package": """
The path to "sphere-aae-package-config.json" which is used for package build.
See "https://github.com/sphere-aae/sphere-aae/blob/main/ios/SphereAaeChat/sphere-aae-package-config.json" as an example.
""".strip(),
    "sphere_aae_source_dir": """
The source code path to Astro Agent Edge (AAE).
""".strip(),
    "output_package": """
