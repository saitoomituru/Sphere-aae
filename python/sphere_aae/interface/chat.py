...
from sphere_aae.serve.engine import SphereAaeEngine
from sphere_aae.serve.engine_base import _query_engine_metrics
from sphere_aae.support import argparse
from sphere_aae.support.config import ConfigOverrideBase
...
class ChatState:
    ...
    interactive chat. It provides support for sphere_aae chat.
    ...
    .. code:: python
        ...
        from sphere_aae.interface.chat import ChatState

        def chat_with_engine(model):
            # hookup with SphereAaeEngine
            ChatState(SphereAaeEngine(model)).chat()

        def chat_with_server(model):
            # hookup with AsyncSphereAaeEngine backed api server
    ...
    # Underlying engine
    engine: Union[JSONFFIEngine, SphereAaeEngine]
    last_finished_request_usage: Optional[openai_api_protocol.CompletionUsage]

    def __init__(self, engine: Union[JSONFFIEngine, SphereAaeEngine]):
        self.engine = engine
        self.history = []
    ...
    def process_system_prompts(self):
        ...
        # TODO(sphere-aae-team): possibly leverage debug option
