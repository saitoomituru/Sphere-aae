...
from sphere_aae.support.auto_device import device2str
from sphere_aae.support.constants import (
    SPHERE_AAE_DSO_SUFFIX,
    SPHERE_AAE_JIT_POLICY,
    SPHERE_AAE_HOME,
    SPHERE_AAE_TEMP_DIR,
)
from sphere_aae.support.style import blue, bold
...
def jit(  # pylint: disable=too-many-locals,too-many-statements
    ...
    *,
    ...
    if SPHERE_AAE_JIT_POLICY == "OFF":
        raise RuntimeError("JIT is disabled by SPHERE_AAE_JIT_POLICY=OFF")

    with open(model_path / "sphere-aae-chat-config.json", "r", encoding="utf-8") as in_file:
        sphere_aae_chat_config = json.load(in_file)
    model_type = sphere_aae_chat_config.pop("model_type")
    quantization = sphere_aae_chat_config.pop("quantization")
    lib_suffix = SPHERE_AAE_DSO_SUFFIX if device not in ["iphone", "android"] else "tar"
    ...
    def _get_model_config() -> Dict[str, Any]:
        model_config = sphere_aae_chat_config.pop("model_config")
        model_config.update(sphere_aae_chat_config)
        for field in dataclasses.fields(ModelConfigOverride):
            value = overrides.get(field.name, None)
            if value is not None:
    ...
    dst = SPHERE_AAE_HOME / "model_lib" / f"{hash_value}.{lib_suffix}"
    if dst.is_file() and SPHERE_AAE_JIT_POLICY in ["ON", "READONLY"]:
        logger.info("Using cached model lib: %s", bold(str(dst)))
        return JITResult(str(dst), system_lib_prefix)
    if SPHERE_AAE_JIT_POLICY == "READONLY":
        raise RuntimeError(
            "No cached model lib found, and JIT is disabled by SPHERE_AAE_JIT_POLICY=READONLY"
        )
    _run_jit(
        opt=hash_key["opt"],
