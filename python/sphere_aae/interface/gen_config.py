"""Generator of sphere-aae-chat-config.json and tokenizer configuration."""
...
from sphere_aae.model import Model
...
from sphere_aae.support import convert_tiktoken, logging
from sphere_aae.support.style import bold, green, red
from sphere_aae.tokenizers import Tokenizer
...
def apply_system_defaults_for_missing_fields(sphere_aae_chat_config: SphereAaeChatConfig) -> None:
    """Apply system default value."""
    for key, value in sphere_aae_chat_config.get_system_defaults_for_missing_fields().items():
        setattr(sphere_aae_chat_config, key, value)
...
def gen_config(  # pylint: disable=too-many-locals,too-many-arguments,too-many-branches,too-many-statements
    ...
    # Step 1. Initialize `sphere-aae-chat-config.json` using `config.json`
    ...
    sphere_aae_chat_config = SphereAaeChatConfig(
    ...
    for generation_config_filename in ["generation_config.json", "config.json"]:
        ...
        if generation_config.exists():
            ...
            for key, value in generation_config_json.items():
                if hasattr(sphere_aae_chat_config, key) and getattr(sphere_aae_chat_config, key) is None:
                    setattr(sphere_aae_chat_config, key, value)
    ...
    # 3.1. Copy over the files and populate sphere_aae_chat_config
    for filename in TOKENIZER_FILES:
        file = config.parent / filename
        if file.exists():
            sphere_aae_chat_config.tokenizer_files.append(filename)
    ...
    if tokenizer_model_file.exists() and (not tokenizer_json_file.exists()):
        ...
        try:
            ...
            sphere_aae_chat_config.tokenizer_files.append("tokenizer.json")
    ...
    if (not tokenizer_json_file.exists()) and list(config.parent.glob("*.tiktoken")):
        try:
            ...
            convert_tiktoken.convert_tiktoken(
                str(config.parent), str(output), sphere_aae_chat_config.context_window_size
            )
            sphere_aae_chat_config.tokenizer_files.append("tokenizer.json")
            sphere_aae_chat_config.tokenizer_files.append("vocab.json")
            sphere_aae_chat_config.tokenizer_files.append("merges.txt")
            sphere_aae_chat_config.tokenizer_files.append("special_tokens_map.json")
    ...
    sphere_aae_chat_config.tokenizer_info = asdict(Tokenizer.detect_tokenizer_info(str(output)))
    logger.info("Detected tokenizer info: %s", sphere_aae_chat_config.tokenizer_info)
    ...
    apply_system_defaults_for_missing_fields(sphere_aae_chat_config)
    ...
    if tokenizer_json_file.exists():
        try:
            ...
            if sphere_aae_chat_config.active_vocab_size != active_vocab_size:
                logger.info(
                    "Overriding active_vocab_size from %d to %d using HF tokenizer",
                    sphere_aae_chat_config.active_vocab_size,
                    active_vocab_size,
                )
                sphere_aae_chat_config.active_vocab_size = active_vocab_size
        except Exception:  # pylint: disable=broad-exception-caught
            logger.warning(
    ...
    with (output / "sphere-aae-chat-config.json").open("w", encoding="utf-8") as out_file:
        json.dump(sphere_aae_chat_config.model_dump(by_alias=True), out_file, indent=2)
