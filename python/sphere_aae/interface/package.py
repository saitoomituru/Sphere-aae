...
from sphere_aae.interface import jit
from sphere_aae.support import download_cache, logging, style
...
def build_model_library(  # pylint: disable=too-many-branches,too-many-locals,too-many-statements
    ...
    if not isinstance(model_entries, list):
        raise ValueError('The "model_list" in "sphere-aae-package-config.json" is expected to be a list.')
    ...
    if not isinstance(model_lib_path_for_prepare_libs, dict):
        raise ValueError(
            'The "model_lib_path_for_prepare_libs" in "sphere-aae-package-config.json" is expected to be '
            "a dict."
        )
    ...
    # - Dump "sphere-aae-app-config.json".
...
def build_android_binding(sphere_aae_source_dir: Path, output: Path) -> None:
    """Build android binding in Astro Agent Edge (AAE)"""
    sphereaae4j_path = sphere_aae_source_dir / "android" / "sphereaae4j"
    ...
    # Build sphereaae4j
    logger.info("Building sphereaae4j")
    subprocess.run([sys.executable, sphereaae4j_path / "prepare_libs.py"], check=True, env=os.environ)
    # Copy built files back to output directory.
    lib_path = output / "lib" / "sphereaae4j"
    ...
    src_path = str(sphereaae4j_path / "src")
    dst_path = str(lib_path / "src")
    logger.info('Copying "%s" to "%s"', src_path, dst_path)
    shutil.copytree(src_path, dst_path)

    src_path = str(sphereaae4j_path / "build.gradle")
    ...
    src_path = str(output / "bundle" / "sphere-aae-app-config.json")
    dst_path = str(lib_path / "src" / "main" / "assets" / "sphere-aae-app-config.json")
    logger.info('Moving "%s" to "%s"', src_path, dst_path)
    shutil.move(src_path, dst_path)


def build_iphone_binding(sphere_aae_source_dir: Path, output: Path) -> None:
    """Build iOS binding in Astro Agent Edge (AAE)"""
    # Build iphone binding
    logger.info("Build iphone binding")
    subprocess.run(
        ["bash", sphere_aae_source_dir / "ios" / "prepare_libs.sh"], check=True, env=os.environ
    )
...
def package(
    package_config_path: Path,
    sphere_aae_source_dir: Path,
    output: Path,
) -> None:
    """Python entrypoint of package."""
    logger.info('Astro Agent Edge (AAE) HOME: "%s"', sphere_aae_source_dir)
    ...
    app_config_path = bundle_dir / "sphere-aae-app-config.json"
    ...
    if device == "android":
        build_android_binding(sphere_aae_source_dir, output)
    elif device == "iphone":
        build_iphone_binding(sphere_aae_source_dir, output)
    else:
