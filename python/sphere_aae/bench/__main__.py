"""Astro Agent Edge (AAE) benchmark main entrance"""
...
import sphere_aae
from sphere_aae.bench.api_endpoint import SUPPORTED_BACKENDS, create_api_endpoint
from sphere_aae.bench.dataset import SUPPORTED_DATASET, Dataset, create_dataset
from sphere_aae.bench.request_processor import (
    MetricAnalyzer,
    RequestProcessor,
    create_pipelines,
)
from sphere_aae.bench.request_record import (
    RequestRecord,
    convert_reports_to_df,
    generate_metrics_summary,
    pretty_print_report,
)
from sphere_aae.cli.serve import EngineConfigOverride
from sphere_aae.serve import EngineConfig
from sphere_aae.support import argparse, logging
...
def _parse_sphere_aae_engine_config(config_str: Optional[str]) -> EngineConfig:
...
def _launch_sphere_aae_server(args: argparse.argparse.Namespace):
    return sphere_aae.serve.PopenServer(
        model=args.tokenizer,
        mode="server",
        model_lib=args.sphere_aae_model_lib,
        enable_tracing=False,
        host=args.host,
        port=args.port,
        engine_config=args.sphere_aae_engine_config,
    )
...
def query_sphere_aae_server_metrics(host: str, port: int):
    """Try to get the Sphere-aae server metrics whenever it exists."""
    try:
        r = requests.post(f"http://{host}:{port}/debug/dump_engine_metrics", json={}, timeout=10)
        if r.status_code == 200:
            print(f"Sphere-aae server metrics: {r.json()}")
...
def main(args: argparse.argparse.Namespace):
    ...
    sphere_aae_server = None
    if args.sphere_aae_model_lib:
        sphere_aae_server = _launch_sphere_aae_server(args)
    ...
    def _main():
        ...
        query_sphere_aae_server_metrics(args.host, args.port)
    ...
    if sphere_aae_server is not None:
        with sphere_aae_server:
            _main()
    else:
        _main()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Astro Agent Edge (AAE) benchmark")
    ...
    parser.add_argument(
        ...
        "This option is only available when benchmarking Sphere-aae server.",
    )
    parser.add_argument(
        "--host",
        type=str,
    ...
    parser.add_argument(
        "--sphere-aae-model-lib",
        type=str,
        help="The model lib path when benchmarking Sphere-aae serve. "
        "When specified, the server is automatic launched and no external server launch is needed.",
    )
    parser.add_argument(
        "--sphere-aae-engine-config",
        type=_parse_sphere_aae_engine_config,
        help="The engine config used when launch Sphere-aae server.",
    )
    parser.add_argument(
        "--cuda-profile",
        default=False,
        ...
        "The --sphere-aae-model-lib path should be provided when enabling this option.",
    )
    parser.add_argument(
        "--debug-dump",
        default=False,
    ...
    parser.add_argument(
        ...
        "-o",
        type=str,
        default="sphere_aae_benchmark.csv",
