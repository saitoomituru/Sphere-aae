...
from sphere_aae.support.config import ConfigBase
from sphere_aae.support.style import green, red
...
def _read_dynamic_shape(shape: List[Union[int, str]], config: Union[Dict, ConfigBase]) -> List[int]:
    ...
    for s in shape:
        if isinstance(s, int):
            ...
            if config is None:
                logger.error(
                    "%s: Encountered dynamic shape %s, need to specify `--sphere-aae-chat-config` for "
...
def main():
    ...
    parser.add_argument(
        "model_lib",
        type=Path,
        help="""The compiled model library. In Astro Agent Edge (AAE), an LLM is compiled to a shared or static
        library (.so or .a), which contains GPU computation to efficiently run the LLM. MLC Chat,
        as the runtime of Astro Agent Edge (AAE), depends on the compiled model library to generate tokens.
        """,
    )
    parser.add_argument(
        "--sphere-aae-chat-config",
        type=Path,
        help="""The `sphere-aae-chat-config.json` file specific to a model variant. This is only required
    ...
    try:
        ...
        return
    # Load sphere_aae_chat_config if provided
    cfg = None
    if parsed.sphere_aae_chat_config:
        sphere_aae_chat_config_path = Path(parsed.sphere_aae_chat_config)
        if not sphere_aae_chat_config_path.exists():
            raise ValueError(f"{sphere_aae_chat_config_path} does not exist.")
        with open(sphere_aae_chat_config_path, "r", encoding="utf-8") as config_file:
