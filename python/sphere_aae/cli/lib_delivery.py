...
class ModelInfo:  # pylint: disable=too-many-instance-attributes
    ...
    # and `tensor_parallel_shards in sphere-aae-chat-config.json
...
class DeferredScope:
    ...
    def create_temp_dir(self) -> Path:
        ...
        temp_dir = tempfile.mkdtemp(dir=SPHERE_AAE_TEMP_DIR)
...
def _run_compilation(model_info: ModelInfo, repo_dir: Path) -> bool:
    ...
    succeeded = True
    with tempfile.TemporaryDirectory(dir=SPHERE_AAE_TEMP_DIR) as temp_dir:
...
def main():
    ...
    parser = ArgumentParser("Astro Agent Edge (AAE) continuous library delivery")
    parser.add_argument(
        "--spec",
        type=_load_spec,
        required=True,
