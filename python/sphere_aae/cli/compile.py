...
from sphere_aae.interface.compile import (  # pylint: disable=redefined-builtin
    ModelConfigOverride,
    OptimizationFlags,
    compile,
)
...
from sphere_aae.support.auto_config import (
    detect_sphere_aae_chat_config,
    detect_model_type,
    detect_quantization,
)
from sphere_aae.support.auto_target import detect_system_lib_prefix, detect_target_and_host


def main(argv):
    """Parse command line arguments and call `sphere_aae.compiler.compile`."""
    ...
    parser = ArgumentParser("sphere_aae compile")
    parser.add_argument(
        "model",
        type=detect_sphere_aae_chat_config,
    ...
    parser.add_argument(
        ...
        + " (default: look up sphere-aae-chat-config.json, choices: %(choices)s)",
    )
    parser.add_argument(
        "--model-type",
        type=str,
