"""Continuous model delivery for Astro Agent Edge (AAE) models."""
...
def _clone_repo(model: Union[str, Path], hf_local_dir: Optional[str]) -> str:
    ...
    sphere_aae_prefix = next(p for p in prefixes if model.startswith(p))
    if sphere_aae_prefix:
        repo_name = model[len(sphere_aae_prefix) :]
...
def _main(  # pylint: disable=too-many-locals, too-many-arguments
    ...
    for task_index, task in enumerate(delivery_diff.tasks, 1):
        ...
        default_destination = (
            delivery_diff.default_destination or "{username}/{model_id}-{quantization}-AAE"
        )
        for quantization in quantizations:
            ...
            output_dir = os.path.join(
                output, f"{model_info.model_id}-{model_info.quantization}-AAE"
            )
...
def main():
    ...
    parser = ArgumentParser("Astro Agent Edge (AAE) continuous model delivery")
    parser.add_argument(
        "--username",
        type=str,
        required=True,
