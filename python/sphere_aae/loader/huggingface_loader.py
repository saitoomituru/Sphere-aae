...
from sphere_aae.support.preshard import _sharded_param_name
from sphere_aae.support.style import bold
...
class HuggingFaceLoader:  # pylint: disable=too-few-public-methods
    ...
    def load(
        ...
        """
        sphere_aae_names = _loading_order(self.extern_param_map, self.torch_to_path)
        for sphere_aae_name in tqdm(sphere_aae_names):
            param = self._load_sphere_aae_param(sphere_aae_name, device=device)
            # Apply quantization if needed, in this case the original parameter may become
            # multiple quantized parameters.
            for name, loader_param in self._load_or_quantize(sphere_aae_name, param, device):
    ...
    def _load_sphere_aae_param(self, sphere_aae_name: str, device: Optional[Device]) -> Tensor:
        torch_names = self.extern_param_map.param_map[sphere_aae_name]
        ...
        with self.stats.timer("map_time_sec"):
            param = self.extern_param_map.map_func[sphere_aae_name](*torch_params)
        if device:
            return as_tensor(param, device=device)
        return as_tensor(param)

    def _load_or_quantize(self, sphere_aae_name, param, device: Device):
        if self.quantize_param_map and sphere_aae_name in self.quantize_param_map.param_map:
            with self.stats.timer("quant_time_sec"):
                q_names = self.quantize_param_map.param_map[sphere_aae_name]
                q_params = self.quantize_param_map.map_func[sphere_aae_name](param)
                device.sync()
            ...
            logger.info(
                ...
                bold(sphere_aae_name),
                param.shape,
                param.dtype,
            )
            device.sync()
            yield sphere_aae_name, param
...
def _loading_order(param_map: ExternMapping, torch_to_path: Dict[str, Path]) -> List[str]:
    ...
    torch_to_sphere_aae = defaultdict(list)
    for sphere_aae_name, torch_names in param_map.param_map.items():
        for torch_name in torch_names:
            torch_to_sphere_aae[torch_name].append(sphere_aae_name)
    # Step 3. Construct the ordering that ensures file locality
    order = OrderedDict()
    for _, torch_names in path_to_torch.items():
        for torch_name in torch_names:
            for sphere_aae_name in torch_to_sphere_aae[torch_name]:
                if sphere_aae_name not in order:
                    order[sphere_aae_name] = 1
