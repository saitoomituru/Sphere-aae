"""Parameter mapping for converting different LLM implementations to Astro Agent Edge (AAE)."""
...
class ExternMapping:
    """Mapping from a parameter name in Astro Agent Edge (AAE)'s model definition to its potential source,
    ...
    unused_params : Set[str]
        Parameter names in the source weights that are not used in the Astro Agent Edge (AAE) model definition.
    """
...
class QuantizeMapping:
    """Mapping from a parameter in Astro Agent Edge (AAE)'s model definition to its eventual names and values after
    ...
    Notes
    -----
    There are two forms of weight conversion in Astro Agent Edge (AAE), one is A) on-the-fly quantization to the
