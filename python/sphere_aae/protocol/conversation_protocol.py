"""The standard conversation protocol in Astro Agent Edge (AAE)"""
...
class Conversation(BaseModel):
    ...
    def as_prompt(self, config=None) -> List[Any]:
        ...
        Returns
        -------
        prompts : List[Union[str, "sphere_aae.serve.data.Data"]]
...
def _combine_consecutive_messages(messages: List[Any]) -> List[Any]:
    ...
    Parameters
    ----------
    messages : List[Union[str, "sphere_aae.serve.data.Data"]]
        The input messages to be combined.
        We use Any in the signature to avoid cyclic import.

    Returns
    -------
    updated_messages : List[Union[str, "sphere_aae.serve.data.Data"]]
