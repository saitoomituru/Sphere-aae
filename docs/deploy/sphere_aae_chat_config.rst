.. _configure-sphere-aae-chat-json:

Customize MLC Chat Config
=========================

``sphere-aae-chat-config.json`` is required for both compile-time and runtime, hence serving two purposes:
...
(e.g. `RedPajama chat config <https://huggingface.co/sphere-aae/sphere-aae-chat-RedPajama-INCITE-Chat-3B-v1-q4f16_1/blob/main/sphere-aae-chat-config.json>`__).


.. _struct-sphere-aae-chat-conv:

Structure of SphereAaeChat Configuration
----------------------------------

Below is the ``sphere-aae-chat-config.json`` file corresponding to Llama2 model:
...
.. note::
  Fields in the first part of ``sphere-aae-chat-config.json`` (e.g. ``context-window-size``)
...
``conv_template``
  .. note::
    Legacy ``sphere-aae-chat-config.json`` may specify a string for this field to look up a registered conversation
    template. It will be deprecated in the future. Re-generate config using the latest version of sphere_aae
...
Sphere-aae provided a set of pre-defined conversation templates, which you can directly use by
...
Please refer to `conversation_template <https://github.com/sphere-aae/sphere-aae/blob/main/python/sphere_aae/conversation_template>`_ directory for the full list of supported templates and their implementations.
