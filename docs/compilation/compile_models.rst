...
To run a model with Astro Agent Edge (AAE) in any platform, we need:

1. **Model weights** converted to MLC format (e.g. `RedPajama-INCITE-Chat-3B-v1-q4f16_1-AAE <https://huggingface.co/sphere-aae/RedPajama-INCITE-Chat-3B-v1-q4f16_1-AAE/tree/main>`__.)
2. **Model library** that comprises the inference logic

This page describes how to compile a model library with Astro Agent Edge (AAE). Model compilation optimizes
...
This replicates :ref:`convert-weights-via-AAE`, see that page for more details.
...
2. Generate sphere-aae-chat-config and compile
---------------------------------------
...
.. tabs::
    ...
    .. group-tab:: iOS/iPadOS
        ...
        .. code:: shell
            ...
            # 1. gen_config: generate sphere-aae-chat-config.json and process tokenizers
            sphere_aae gen_config ./dist/models/RedPajama-INCITE-Chat-3B-v1/ --quantization q4f16_1 \
    ...
    .. group-tab:: WebGPU
        ...
        .. note::
            To compile for webgpu, you need to build from source when installing ``sphere_aae``. Besides, you also need to follow :ref:`install-web-build`.
...
.. note::
    ...
    However, adding your own template would require you :ref:`build sphere_aae from source <aaechat_build_from_source>`
...
.. tabs::
    ...
    .. group-tab:: Linux - CUDA
        ...
        .. code:: shell
            ...
            >>> engine = SphereAaeEngine(model="./dist/RedPajama-INCITE-Chat-3B-v1-q4f16_1-AAE",
...
generalized to any model variant, as long as sphere-aae supports the architecture.
...
.. tabs::
    ...
    .. tab:: Model: Llama-2-7B
        ...
        .. code:: shell
            ...
            sphere_aae convert_weight ./dist/models/Llama-2-7b-chat-hf/ --quantization q4f16_1 -o dist/Llama-2-7b-chat-hf-q4f16_1-AAE

        Afterwards, run the following command to generate sphere-aae config and compile the model.
        ...
        .. tabs::
            ...
            .. tab:: Target: CUDA
                ...
                .. code:: shell
                    ...
                    # 1. gen_config: generate sphere-aae-chat-config.json and process tokenizers
                    sphere_aae gen_config ./dist/models/Llama-2-7b-chat-hf/ --quantization q4f16_1 \
                        --conv-template llama-2 -o dist/Llama-2-7b-chat-hf-q4f16_1-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/Llama-2-7b-chat-hf-q4f16_1-AAE/sphere-aae-chat-config.json \
            ...
            .. tab:: WebGPU
                ...
                .. code:: shell
                    ...
                    # 1. gen_config: generate sphere-aae-chat-config.json and process tokenizers
                    sphere_aae gen_config ./dist/models/Llama-2-7b-chat-hf/ --quantization q4f16_1 \
                        --context-window-size 2048 --conv-template llama-2 -o dist/Llama-2-7b-chat-hf-q4f16_1-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/Llama-2-7b-chat-hf-q4f16_1-AAE/sphere-aae-chat-config.json \
            ...
            .. tab:: iPhone/iPad
                ...
                .. code:: shell
                    ...
                    # 1. gen_config: generate sphere-aae-chat-config.json and process tokenizers
                    sphere_aae gen_config ./dist/models/Llama-2-7b-chat-hf/ --quantization q4f16_1 \
                        --conv-template llama-2 --context-window-size 768 -o dist/Llama-2-7b-chat-hf-q4f16_1-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/Llama-2-7b-chat-hf-q4f16_1-AAE/sphere-aae-chat-config.json \
    ...
    .. tab:: Mistral-7B-Instruct-v0.2
        ...
        .. code:: shell
            ...
            sphere_aae convert_weight ./dist/models/Mistral-7B-Instruct-v0.2/ --quantization q4f16_1 \
                -o dist/Mistral-7B-Instruct-v0.2-q4f16_1-AAE
        ...
        .. tabs::
            ...
            .. tab:: Target: CUDA
                ...
                .. code:: shell
                    ...
                    # 1. gen_config: generate sphere-aae-chat-config.json and process tokenizers
                    sphere_aae gen_config ./dist/models/Mistral-7B-Instruct-v0.2/ --quantization q4f16_1 \
                        --conv-template mistral_default -o dist/Mistral-7B-Instruct-v0.2-q4f16_1-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/Mistral-7B-Instruct-v0.2-q4f16_1-AAE/sphere-aae-chat-config.json \
            ...
            .. tab:: Android
                ...
                .. code:: shell
                    ...
                    # 1. gen_config: generate sphere-aae-chat-config.json and process tokenizers
                    sphere_aae gen_config ./dist/models/Mistral-7B-Instruct-v0.2/ --quantization q4f16_1 \
                        --conv-template mistral_default --sliding-window-size 1024 --prefill-chunk-size 128 -o dist/Mistral-7B-Instruct-v0.2-q4f16_1-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/Mistral-7B-Instruct-v0.2-q4f16_1-AAE/sphere-aae-chat-config.json \
    ...
    .. tab:: Other models
        ...
        .. code:: shell
            ...
            sphere_aae convert_weight ./dist/models/HF_MODEL/ --quantization q4f16_1 -o dist/OUTPUT-AAE
        ...
        .. tabs::
            ...
            .. tab:: Target: CUDA
                ...
                .. code:: shell
                    ...
                    # 1. gen_config: generate sphere-aae-chat-config.json and process tokenizers
                    sphere_aae gen_config ./dist/models/HF_MODEL/ --quantization q4f16_1 --conv-template CONV_TEMPLATE -o dist/OUTPUT-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/OUTPUT-AAE/sphere-aae-chat-config.json --device cuda -o dist/libs/OUTPUT-cuda.so
            ...
            .. tab:: Metal
                ...
                .. code:: shell
                    ...
                    sphere_aae gen_config ./dist/models/HF_MODEL/ --quantization q4f16_1 --conv-template CONV_TEMPLATE -o dist/OUTPUT-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/OUTPUT-AAE/sphere-aae-chat-config.json --device metal -o dist/libs/OUTPUT-metal.so
                ...
                .. code:: shell
                    ...
                    sphere_aae gen_config ./dist/models/HF_MODEL/ --quantization q4f16_1 --conv-template CONV_TEMPLATE -o dist/OUTPUT-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/OUTPUT-AAE/sphere-aae-chat-config.json --device metal -o dist/libs/OUTPUT-metal_x86_64.dylib
            ...
            .. tab:: Vulkan
                ...
                .. code:: shell
                    ...
                    sphere_aae gen_config ./dist/models/HF_MODEL/ --quantization q4f16_1 --conv-template CONV_TEMPLATE -o dist/OUTPUT-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/OUTPUT-AAE/sphere-aae-chat-config.json --device vulkan -o dist/libs/OUTPUT-vulkan.so
                ...
                .. code:: shell
                    ...
                    sphere_aae gen_config ./dist/models/HF_MODEL/ --quantization q4f16_1 --conv-template CONV_TEMPLATE -o dist/OUTPUT-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/OUTPUT-AAE/sphere-aae-chat-config.json --device vulkan -o dist/libs/OUTPUT-vulkan.dll
            ...
            .. tab:: WebGPU
                ...
                .. code:: shell
                    ...
                    sphere_aae gen_config ./dist/models/HF_MODEL/ --quantization q4f16_1 --conv-template CONV_TEMPLATE -o dist/OUTPUT-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/OUTPUT-AAE/sphere-aae-chat-config.json --device webgpu -o dist/libs/OUTPUT-webgpu.wasm
            ...
            .. tab:: iPhone/iPad
                ...
                .. code:: shell
                    ...
                    # 1. gen_config: generate sphere-aae-chat-config.json and process tokenizers
                    sphere_aae gen_config ./dist/models/HF_MODEL/ --quantization q4f16_1 --conv-template CONV_TEMPLATE \
                        --context-window-size 768 -o dist/OUTPUT-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/OUTPUT-AAE/sphere-aae-chat-config.json --device iphone -o dist/libs/OUTPUT-iphone.tar
            ...
            .. tab:: Android
                ...
                .. code:: shell
                    ...
                    sphere_aae gen_config ./dist/models/HF_MODEL/ --quantization q4f16_1 --conv-template CONV_TEMPLATE \
                        --context-window-size 768 -o dist/OUTPUT-AAE/
                    # 2. compile: compile model library with specification in sphere-aae-chat-config.json
                    sphere_aae compile ./dist/OUTPUT-AAE/sphere-aae-chat-config.json --device android -o dist/libs/OUTPUT-android.tar
...
``sphere-aae-chat-config.json``, and compile the model. This section describes the list of options that can be used
during compilation.
...
--CONFIG                            It can be one of the following:
                                    ...
                                    `here <https://github.com/sphere-aae/sphere-aae/blob/main/python/sphere_aae/compiler/model/model.py>`_.
...
In order to compile a model, we first need to generate the ``sphere-aae-chat-config.json``. This file contains specifications
...
--conv-template CONV_TEMPLATE                   Conversation template. It depends on how the model is tuned. Use "LM" for vanilla base model
                                                ...
                                                `here <https://github.com/sphere-aae/sphere-aae/blob/main/python/sphere_aae/model/model.py>`_.
...
--output OUTPUT                                 The output directory for generated configurations, including `sphere-aae-chat-config.json` and tokenizer configuration.

3. Compile Model Library
^^^^^^^^^^^^^^^^^^^^^^^^

After generating ``sphere-aae-chat-config.json``, we can compile the model into a model library (files ending in ``.so``, ``.tar``, etc. that contains
...
--MODEL                                     A path to ``sphere-aae-chat-config.json``, or an MLC model directory that contains ``sphere-aae-chat-config.json``.
...
--model-type MODEL_TYPE                     Model architecture such as "llama". If not set, it is inferred from ``sphere-aae-chat-config.json``.
...
--opt OPT                                   Optimization flags. Astro Agent Edge (AAE) maintains a predefined set of optimization flags,
...
--overrides OVERRIDES                       Model configuration override. Configurations to override ``sphere-aae-chat-config.json``. Supports
